  ...
  return (
    <Popover isOpen={isOpen} onClose={close} placement="bottom-start" autoFocus={false} matchWidth isLazy>
      <PopoverTrigger>
        ...
        >
          <Text flex={1} noOfLines={1} color={selectedOption ? undefined : placeholderColor} fontSize="sm">
            {selectedOption ? selectedOption.name : placeholder}
          </Text>
          {selectedOption?.badge && <Text fontSize="xs" color="subtleText" mr={2}>{selectedOption.badge}</Text>}
          {allowClear && selectedOption && !isDisabled && (
            <IconButton icon={<FiX />} aria-label="Clear selection" size="xs" variant="ghost"
      ...
      <PopoverContent w="100%">
        <PopoverBody p={2}>
          <InputGroup size="sm" mb={2}>
            ...
            {search && (
              <InputRightElement>
                <IconButton icon={<FiX />} aria-label="Clear search" size="xs" variant="ghost" onClick={() => setSearch('')} />
              </InputRightElement>
            )}
          </InputGroup>
